...
public class CustomerDaoJdbcTemplateImpl implements CustomerDao {
    private static final String CREATE_CUSTOMER_TABLE_SQL = "CREATE TABLE CUSTOMER (CUSTOMER_ID INTEGER GENERATED BY DEFAULT AS IDENTITY (START WITH 1), COMPANY_NAME VARCHAR(255), EMAIL VARCHAR(255), TELEPHONE VARCHAR(255), NOTES VARCHAR(255))";
    private static final String CREATE_CALL_TABLE_SQL = "CREATE TABLE TBL_CALL (ID INTEGER GENERATED BY DEFAULT AS IDENTITY (START WITH 1), CUSTOMER_ID INT, TIME_AND_DATE TIMESTAMP, NOTES VARCHAR(255))";
    private static final String CREATE_CUSTOMER_SQL = "INSERT INTO CUSTOMER (COMPANY_NAME, EMAIL, TELEPHONE, NOTES) " +
        "VALUES (?, ?, ?, ?)";
    ...
    public void addCall(Call newCall, String customerId) throws RecordNotFoundException {
        try {
            jdbcTemplate.update(CREATE_CALL_SQL, customerId, new Timestamp(newCall.getTimeAndDate().getTime()), newCall.getNotes());
        } catch (Exception e) {
            e.printStackTrace();
            throw new RecordNotFoundException();
        }
    ...
    static class CallRowMapper implements RowMapper<Call> {
        ...
        public Call mapRow(ResultSet rs, int arg1) throws SQLException {
            String notes = rs.getString("NOTES");
            Date timeAndDate = new Date(rs.getTimestamp("TIME_AND_DATE").getTime());
